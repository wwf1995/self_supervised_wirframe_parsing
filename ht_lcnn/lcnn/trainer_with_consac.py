...
class Trainer(object):
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, (image, meta, target) in enumerate(self.val_loader):
                ...
                # scores = H["score"]
                '''
                lines = H["lines"][0].detach().numpy() / 128 * image.shape[:2]
                scores = H["score"][0].detach().numpy()
                diag = (image.shape[0] ** 2 + image.shape[1] ** 2) ** 0.5
                nlines, nscores = postprocess(lines, scores, diag * 0.01, 0, False)
                lines = []
                for i in range(len(nlines)):
                    if scores[i] > threshold:
                        lines.append(nlines[i])

                if len(lines) < 32:
                    for i in range(32 - len(lines)):
                        lines.append(lines[i])
                line_segments = np.array(lines)
                line_segments[:,:,[0,1]] = line_segments[:,:,[1,0]]
                data = torch.from_numpy(line_segments.reshape((self.batch_size,-1,4)))'''
                ...
                line_segments_scaled_h = np.zeros((line_segments_scaled.shape[0], line_segments_scaled.shape[1], 12),
                                                  dtype=np.float32)
                for i in range(lines.shape[0]):
                    nlines = np.zeros((line_segments_scaled.shape[1], 13), dtype=np.float32)
                    for li in range(lines.shape[1]):
                        p1 = np.array([line_segments_scaled[i, li, 0, 0], line_segments_scaled[i, li, 0, 1], 1])
                        p2 = np.array([line_segments_scaled[i, li, 1, 0], line_segments_scaled[i, li, 1, 1], 1])
                        centroid = 0.5 * (p1 + p2)
                        line = np.cross(p1, p2)
                        ...
                        nlines[li, 3:6] = p2
                        nlines[li, 6:9] = line
                        nlines[li, 9:12] = centroid
                        nlines[li, 12] = np.linalg.norm(np.cross(p1, p2)[0:2])

                    valid_line = nlines[nlines[:, 12] > 0]
                    nlines[nlines[:, 12] == 0] = valid_line[np.random.randint(valid_line.shape[0])]
                    line_segments_scaled_h[i] = nlines[:, 0:12]
                    del nlines, valid_line

                data = torch.from_numpy(line_segments_scaled_h)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (image, meta, target) in enumerate(self.train_loader):
            ...
            for i in range(lines.shape[0]):
                #nlines = torch.zeros((line_segments_scaled.shape[1], 13), dtype=torch.float)
                nlines = []
                for li in range(lines.shape[1]):
                    p1 = torch.cat((line_segments_scaled[i, li, 0],torch.tensor([1])),0)
                    p2 = torch.cat((line_segments_scaled[i, li, 1],torch.tensor([1])),0)
                    centroid = 0.5 * (p1 + p2)
            ...
            #data = torch.tensor([item.cpu().detach().numpy() for item in line_segments_scaled_h]).cuda()
            #data = torch.tensor([item.cpu().detach().numpy() for item in line_segments_scaled_h])
            data = torch.cat(line_segments_scaled_h,0)
            print("data shape",data.shape)
            print("data is leaf: ",data.is_leaf)
            register_hook(data,"data")

            num_data = num_actual_line_segments
            #gt_models = input_dict["target"]['vpts']
            num_models = 3
